...
FILE *file;
const char file_txt[] = "/labhop.txt";
const char file_wav[] = "/labhop.wav";
char pathFile[64];
...
#define RECORD_TIME (45 * 60) // Seconds
...
static void upload()
{
  ...
  if (status_code == 200 || status_code == 201)
  {
     int size_reponse = 1024;
      char *response = malloc(size_reponse);
      if (response == NULL) {
        ESP_LOGE(TAG, "Failed to allocate memory for response");
        return;
    }
      int len = esp_http_client_read(client, response, size_reponse - 1);
      if (len > 0)
      {
          response[len] = '\0';
          ESP_LOGI(TAG, "Response from Gemini: %s", response);
  
          cJSON *root = cJSON_Parse(response);
          if (root) {
              const char *uri_found = find_value_by_key_recursive(root, "text");
              if (uri_found) {
                  strncpy(response, uri_found, size_reponse - 1);
                  response[size_reponse - 1] = '\0';
                  printf("SERVER_RESPONE: ==================== Transcription ====================\n");
                  printf("Text: %s", response);
                  printf("SERVER_RESPONE: ==================== END ====================\n");
                  save_file_txt(response);
                  //smtp_client_task(NULL);
                 // xTaskCreate(&smtp_client_task, "smtp_client_task", 1024 * 5, NULL, 5, NULL);
              } else {
                  ESP_LOGW(TAG, "URI not found in response");
              }
              cJSON_Delete(root);
...
static void wifi_event_handler(void *event_handler_arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
  ...
  case IP_EVENT_STA_GOT_IP:
    printf("WiFi got IP ... \n\n");
   xTaskCreate(post_task, "post_task", 1024 * 10, NULL, 10, NULL);
    break;
  default:
    break;
  }
...
void app_main(void)
{
  ...
  i2sInit();
  xTaskCreate(i2s_adc, "i2s_adc",1024 * 20, NULL, 10, NULL);
